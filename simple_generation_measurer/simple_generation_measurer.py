from component_types import (
    SimpleGenerationMeasuererEvent
    )


def handle_generation_runner_event(event: SimpleGenerationMeasuererEvent):
    # We won't have our metadata, so build it:
    metadata = event["metadata"]
    population = event["population"]
    metadata["simple_generation_measuerer"]["population"] = population
    metadata["simple_generation_measuerer"]["fitnesses"] = {}
    # Pick an individual:
...
def handle_individual_measurer_event(event: SimpleGenerationMeasuererEvent):
    # For this one, we *SHOULD* have the metadata:
    # Let's take the fitnesses we received and add them to our metadata:
    metadata = event["metadata"]
    population = metadata["simple_generation_measuerer"]["population"]
    saved_fitnesses = metadata["simple_generation_measuerer"]["fitnesses"]
    ...
    if len(remaining) == 0:
        # We're done:
        # Clear our metadata:
        del metadata["simple_generation_measuerer"]
        return {
            "config": event["config"],
            "metadata": metadata,
            "is_done": True,
...
def lambda_handler(event: SimpleGenerationMeasuererEvent, context):
